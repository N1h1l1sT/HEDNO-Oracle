  ...
  FROM [YLIKA_KOSTOL].[dbo].[ΕΡΓΑ]
	...
	INNER JOIN (SELECT ID AS tmpID29, (datepart(year, [ΗΜΕΡ_ΑΙΤΗΣΗΣ]) * SIN(((datepart(dayofyear, [ΗΜΕΡ_ΑΙΤΗΣΗΣ])) * 360) / 366)) AS [DayOfYearCartesY]
				...
				ON tmp29.tmpID29 = [ΕΡΓΑ].ID
															...
															--432646
  WHERE (([ΔΕΗ_ΠΕΛΑΤΗΣ] <> 1 OR [ΔΕΗ_ΠΕΛΑΤΗΣ] IS NULL) AND --249528 --Getting only Clients --1=DEH
		 ([ΑΚΥΡΩΘΕΝ] = 0 OR [ΑΚΥΡΩΘΕΝ] IS NULL) AND --239555 --Akyrothen <> 0 = false record, ergo non needed
		 ...
		 ([Label] = 1 OR DATEADD(MONTH,2,[ΗΜΕΡ_ΑΙΤΗΣΗΣ]) < '2016-07-06') AND --213535  --[Instead of '2016-07-06', it should be GETDATE() on the real HEDNO's server]  --Getting the results with actual label of 0 or 1, not as of yet undecided
		 [Xaraktirismos_Ergou] IS NOT NULL AND --204771 --The database encompasses a wide range of things but we only care for projects. Those are the 41,D,42,M. The NOT NULL means that that's all the query retrieves
		 [Skopos_Ergou] IS NOT NULL AND --190401 --The feature's been engineered so that only rows with a value on this are relevant
		 
		 [ΗΜΕΡΕΣ_ΜΕΛΕΤΗΣ] >= 0 AND --182691 --A study cannot have ended before it even began, hence erroneous data (noise) are being cleaned
		 [ΚΟΣΤΟΣ_ΕΡΓΑΤΙΚΩΝ_ΚΑΤΑΣΚΕΥΗΣ] IS NOT NULL AND --159326 --Cost is a critical variable, so it must be filled in
		 [ΚΟΣΤΟΣ_ΥΛΙΚΩΝ_ΚΑΤΑΣΚΕΥΗΣ] IS NOT NULL AND --155915 --Cost is a critical variable, so it must be filled in
		 [ΚΟΣΤΟΣ_ΚΑΤΑΣΚΕΥΗΣ] IS NOT NULL AND --155740 --Cost is a critical variable, so it must be filled in
		 [ΚΟΣΤΟΣ_ΕΡΓΟΛΑΒΙΚΩΝ_ΕΠΙΔΟΣΗΣ] IS NOT NULL AND --153739 --Cost is a critical variable, so it must be filled in

		 [ΚΟΣΤΟΣ_ΕΡΓΑΤΙΚΩΝ_ΚΑΤΑΣΚΕΥΗΣ] > 0 AND --153577 --A negative cost points to a dismantlement, which are irrelevant projects
		 [ΚΟΣΤΟΣ_ΥΛΙΚΩΝ_ΚΑΤΑΣΚΕΥΗΣ] > 0 AND --150520 --A negative cost points to a dismantlement, which are irrelevant projects
		 [ΚΟΣΤΟΣ_ΚΑΤΑΣΚΕΥΗΣ] > 0 AND --150486 --A negative cost points to a dismantlement, which are irrelevant projects
		 [ΚΟΣΤΟΣ_ΕΡΓΟΛΑΒΙΚΩΝ_ΕΠΙΔΟΣΗΣ] > 0 AND --150485 --A negative cost points to a dismantlement, which are irrelevant projects

		 [MONADA] IS NOT NULL AND --150485
		 [ΚΑΤΗΓΟΡΙΑ] IS NOT NULL AND --146759
		 [ΗΜΕΡΕΣ_ΜΕΛΕΤΗΣ] IS NOT NULL AND --146759
		 [ΣΥΝΕΡΓΕΙΟ_ΜΕΛΕΤΗΣ] IS NOT NULL AND --146722
		 [ΕΚΤΑΣΗ_ΕΡΓΟΥ] IS NOT NULL AND --134618
		 [ΑΝΑΓΚΗ_ΥΣ] IS NOT NULL AND --134618

		 [ΗΜΕΡ_ΑΙΤΗΣΗΣ] IS NOT NULL AND --134480
		 [ΗΜΕΡ_ΚΑΤΑΧΩΡΗΣΗΣ] IS NOT NULL AND --134479
		 [ΗΜΕΡ_ΑΝΑΓΓΕΛΙΑΣ] IS NOT NULL AND --133122
		 [ΗΜΕΡ_ΜΕΛΕΤΗΣ] IS NOT NULL --AND --133098

		 --[SAP_ΤΥΠΟΣ_ΠΕΛΑΤΗ] IS NOT NULL AND --28207
		 --[SAP_ΕΙΔΟΣ_ΑΙΤΗΜΑΤΟΣ] IS NOT NULL --AND --21428
		 )