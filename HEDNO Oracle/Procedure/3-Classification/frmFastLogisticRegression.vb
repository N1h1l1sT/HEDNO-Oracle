...
Public Class frmFastLogisticRegression
    Public strLanguage_FastLogisticRegression As String()
    Public strLanguage_FastLogisticRegression_Tips As String()
...
#Region "Functions"
    Private Sub UpdateCombinations()
        If Not isInitialising Then
            ...
            If clbColumns.CheckedIndices.Count <= 1 Or Not chkColumnsCombinations.Checked Then
                lblCombinationsCount.Text = "1" & strLanguage_FastLogisticRegression(19) ' Combination
            Else
                If chkUpToNGramsN.Checked Then
                    ...
                    Next
                    lblCombinationsCount.Text = (CombSum).ToString("n0") & strLanguage_FastLogisticRegression(20) ' Combinations
                Else
                    lblCombinationsCount.Text = (NChooseK(clbColumns.CheckedIndices.Count, CInt(txtNGrams.Text))).ToString("n0") & strLanguage_FastLogisticRegression(20) ' Combinations
                End If
            End If
        End If
    End Sub
    ...
    Private Sub CheckModelExists()
        If File.Exists(doProperFileName(strModelsPath & "FastLogisticRegressionModel.RDS")) Then
            ModelExists = True
...
#End Region
    ...
    Private Shadows Sub FormClosing(ByVal sender As Object, ByVal e As ComponentModel.CancelEventArgs) Handles MyBase.Closing
        Try
            If FuncInProgress.Count <> 0 Then
                ...
                If MsgBox(sa(strLanguage_FastLogisticRegression(30), vbCrLf, ArrayBox(False, ";", 0, True, FuncInProgress)), MsgBoxStyle.YesNoCancel) <> MsgBoxResult.Yes Then
                    e.Cancel = True
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub frmFastLogisticRegression_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            'initialization
            Call FastLogisticRegression_Language(Me)
            frmSkin(Me, False)
            ...
            If Not File.Exists(strXDF & "Training_DS.xdf") OrElse Not File.Exists(strXDF & "Test_DS.xdf") Then
                If File.Exists(strXDF & "Classification_DS.xdf") OrElse File.Exists(strXDF & "Clustering_DS.xdf") Then
                    ...
                    If File.Exists(strXDF & "Classification_DS.xdf") Then
                        If File.Exists(strXDF & "Training_DS.xdf") OrElse File.Exists(strXDF & "Test_DS.xdf") Then
                            ...
                            MsgBox(sa(strLanguage_FastLogisticRegression(33), vbCrLf, strXDF))
                        Else
                            ...
                            MsgBox(sa(strLanguage_FastLogisticRegression(34), vbCrLf, strXDF))
                        End If
                    ...
                    ElseIf File.Exists(strXDF & "Clustering_DS.xdf") Then
                        ...
                        MsgBox(sa(strLanguage_FastLogisticRegression(35), vbCrLf, strXDF))
                    End If
                ...
                Else
                    ...
                    MsgBox(sa(strLanguage_FastLogisticRegression(36), vbCrLf, strXDF, RemMniHotLetter(frmMain.mniClustering), RemMniHotLetter(frmMain.mniFormTrainAndTestSets), RemMniHotLetter(frmMain.mniClassification)))
            ...
            fswModelFileExists.Filter = "FastLogisticRegressionModel.RDS"
    ...
    Private Sub btnRunModel_Click(sender As Object, e As EventArgs) Handles btnRunModel.Click
        Try
            If Not isWorking Then
                ...
                (Not chkUseExistingModel.Checked AndAlso Not chkMakePredictions.Checked AndAlso Not chkSavePredictionModel.Checked) Then
                    ...
                    If FuncInProgress.Count = 0 Then
                        ...
                        pbLoading.Visible = True
                        btnRunModel.Text = strLanguage_FastLogisticRegression(37) '&Cancel
                        ...
                        FuncInProgress.Add(strLanguage_FastLogisticRegression(24)) 'Applying Fast Logistic Regression
                        Try
                            Dim TestColumnNames() As String = {}

                            If RDotNet_Initialization() Then
                                If StopWorking Then
                                    FuncInProgress.Remove(strLanguage_FastLogisticRegression(24)) 'Applying Fast Logistic Regression
                                    Close()
                                    Exit Sub
                                End If
                                ...
                                If CheckedColumnsExistInTest Then
                                    ...
                                    For i As Integer = 0 To ColumnNamesFormula.Count - 1
                                        If StopWorking Then
                                            ...
                                            Exit Sub
                                        End If
                                        WriteToLog({sa("FLR_PredictionReal{0}:  {1}",
                                        ...
                                        If chkColumnsCombinations.Checked OrElse chkUpToNGramsN.Checked Then
                                            ResultsFileName = sa("FastLogisticRegression_Results_{0}.csv", (i + 1))
                                            predVarName_ForMultiROC.Add(sa("FLR_PredictionReal{0}", (i + 1)))
                                        Else
                                            ResultsFileName = "FastLogisticRegression_Results.csv"
                                        End If

                                        Dim SinkFilePathLinux As String = doProperFileNameLinux(strSinkFile)
                                        If RSource({strFunctions & "3.10 Fast Logistic Regression.R"}, , {"{reportProgress}", If(chkreportProgress.Checked, txtReportProgress.Text, "rxGetOption('reportProgress')"),
                                                                                                        ...
                                                                                                        "{6}", "FastLogisticRegressionModel",
                                                                                                        "{7}", If(chksgdInitTol.Checked, txtsgdInitTol.Text, "0"),
                                                                                                        "{8}", If(chkl2Weight.Checked, txtl2Weight.Text, "0"),
                                                                                                        "{9}", If(chkl1Weight.Checked, txtl1Weight.Text, "0"),
                                                                                                        "{10}", If(chkoptTol.Checked, txtoptTol.Text, "1e-07"),
                                                                                                        "{11}", If(chkmemorySize.Checked, txtmemorySize.Text, "20"),
                                                                                                        "{12}", If(chkinitWtsScale.Checked, txtinitWtsScale.Text, "0"),
                                                                                                        "{13}", If(chkmaxIterations.Checked, txtmaxIterations.Text, "100"),
                                                                                                        "{14}", If(chknormalize.Checked, cbnormalize.SelectedItem.ToString, "auto")
                                                                                                }, True) Then
                                            ...
                                            If chkStatisticsMode.Checked Then
                                                If chkShowStatistics.Checked Then
                                                    ...
                                                    If Not LabelPredictionExist Then 'If those don't exist, then the Statistics will crash
                                                        ...
                                                        MsgBox(sa(strLanguage_FastLogisticRegression(38), vbCrLf, RemCtrHotLetter(chkShowStatistics)))
                                                    Else
                                                        Dim StatisticsResults As GenericVector = Rdo.GetSymbol("StatisticsResults").AsList
                                                    End If

                                                    If chkShowStatistics.Checked Then
                                                        Dim StatisticsForm As New frmStatistics With {.SinkFilePath = SinkFilePathLinux, .ClassificationModelName = "FastLogisticRegressionModel"}
                                                        StatisticsForm.Show()
                                                    End If
                                                ...
                                                If chkShowROCCurve.Checked Then
                                                    ...
                                                    If Not PredictionRealExist Then 'If this doesn't exist, then the ROC curve will crash
                                                        '         '{1}' was checked, however the Testing Dataset did not have the needed variables.{0}Have you forgotten applying the Predictions from the Machine Learning Algorithm?
                                                        MsgBox(sa(strLanguage_FastLogisticRegression(38), vbCrLf, RemCtrHotLetter(chkShowROCCurve)))
                                                    End If
                                                End If
                                            End If
                                            ...
                                            If RDSCreatedOutOfNecessity Then MsgBox(sa(strLanguage_FastLogisticRegression(28), vbCrLf, RemCtrHotLetter(chkUseExistingModel)))
                                    ...
                                    If chkColumnsCombinations.Checked AndAlso chkShowROCCurve.Checked Then
                                        ...
                                        If Not RSource({strFunctions & "[Multiple_ROC_Comparisons].R"}, , {"{0}", predVarName,
                                            ...
                                            MsgBox(sa(strLanguage_FastLogisticRegression(39), vbCrLf, strXDF & "Test_DS.xdf", predVarName), MsgBoxStyle.Critical)
                                        End If
                                    End If
                                ...
                                Else
                                    ...
                                    MsgBox(sa(strLanguage_FastLogisticRegression(40), vbCrLf))
                                End If
                    ...
                    Else
                        ...
                        MsgBox(sa(strLanguage_FastLogisticRegression(26), ArrayBox(False, ";", 0, True, FuncInProgress)), MsgBoxStyle.Exclamation) '
                    End If


                Else
                    '         You haven't selected any Dependent Variables, ergo the operation is cancelled
                    MsgBox(sa(strLanguage_FastLogisticRegression(41)), MsgBoxStyle.Exclamation)
                End If
            ...
            Else '#If isWorking
                ...
                btnRunModel.Text = strLanguage_FastLogisticRegression(42) 'Cancelling...
...
#Region "Misc"
    ...
    Private Sub UpdateSelectAllText()
        If (chkShowDataSummary.Checked OrElse Not chkShowDataSummary.Enabled) AndAlso
           ...
           (chkSavePredictionModel.Checked OrElse Not chkSavePredictionModel.Enabled) Then
            ...
            btnSelectAll.Text = strLanguage_FastLogisticRegression(25) 'Unselect &All
        Else
            btnSelectAll.Text = strLanguage_FastLogisticRegression(12) 'Select &All
        End If
    End Sub
    Private Sub clbColumns_SelectedIndexChanged(sender As Object, e As EventArgs) Handles clbColumns.SelectedIndexChanged, clbColumns.DoubleClick
        If clbColumns.CheckedIndices.Count = clbColumns.Items.Count Then
            btnSelectAllColumns.Text = strLanguage_FastLogisticRegression(43) 'Unselect All &Columns
        Else
            btnSelectAllColumns.Text = strLanguage_FastLogisticRegression(29) 'Select &All Columns
        End If
    ...
    Private Sub txtSavePath_Click(sender As Object, e As EventArgs) Handles txtSavePath.Click
        fbdModelPath.Description = strLanguage_FastLogisticRegression(44) 'Where would you like your Model to be saved?
...
#End Region
    ...
    Private Sub tmrLoadColumns_Tick(sender As Object, e As EventArgs) Handles tmrLoadColumns.Tick
        Try
            ...
            Catch ex As Exception
                ...
                MsgBox(sa(strLanguage_FastLogisticRegression(45), vbCrLf, "TrainingColumnNames", strFunctions & "[ColumnNames_For_Classification].R"))
                Close()
                Exit Sub
            End Try
    ...
    Private Sub txtNGrams_Click(sender As Object, e As EventArgs) Handles txtNGrams.Click
        If clbColumns.CheckedIndices.Count > 0 Then
            ...
            If TypeBox(strLanguage_FastLogisticRegression(46), NGram, False,, 1, clbColumns.CheckedIndices.Count,,,,
    ...
    Private Sub txtRoundAt_Click(sender As Object, e As EventArgs) Handles txtRoundAt.Click
        ...
        If TypeBox(strLanguage_FastLogisticRegression(17), CurRoundAt, False,, 1, Integer.MaxValue,,,, RoundAt.ToString) Then
            txtRoundAt.Text = CurRoundAt.ToString
        End If
    End Sub
    ...
    Private Sub fswTrainAndTest_Created_Created_Renamed(sender As Object, e As FileSystemEventArgs) Handles fswTrainAndTest.Renamed, fswTrainAndTest.Deleted, fswTrainAndTest.Created
        If e.Name.EndsWith("Training_DS.xdf") OrElse e.Name.EndsWith("Test_DS.xdf") Then
            ...
            Else
                ...
                ShowMTNonInterruptingMsgbox(Me, sa(strLanguage_FastLogisticRegression(47), vbCrLf), MsgBoxStyle.Exclamation)
    ...
    Private Sub txtsgdInitTol_Click(sender As Object, e As EventArgs) Handles txtsgdInitTol.Click
        Dim sgdInitTol As Double = 0.0#
        If TypeBox("A number greater than 0 will use Stochastic Gradient Descent (SGD) to find the initial parameters with that number as tolerance:", sgdInitTol, False,, 0#, MaxDouble,,,,,,, sgdInitTol.ToString) Then
            txtsgdInitTol.Text = sgdInitTol.ToString
        End If
    End Sub

    Private Sub txtl2Weight_Click(sender As Object, e As EventArgs) Handles txtl2Weight.Click
        Dim l2Weight As Double = 0.0#
        If TypeBox(sa("Type a number for the L2 regularization weight(preferable for data that is not sparse.{0}It pulls large weights towards zero):", vbCrLf), l2Weight, False,, 0#, MaxDouble,,,,,,, l2Weight.ToString) Then
            txtl2Weight.Text = l2Weight.ToString
        End If
    End Sub

    Private Sub txtl1Weight_Click(sender As Object, e As EventArgs) Handles txtl1Weight.Click
        Dim l1Weight As Double = 0.0#
        If TypeBox(sa("Type a number for the L1 regularization weight (can be applied to sparse models, when working with high-dimensional data.{0}It pulls small weights associated features that are relatively unimportant towards 0):", vbCrLf), l1Weight, False,, 0#, MaxDouble,,,,,,, l1Weight.ToString) Then
            txtl1Weight.Text = l1Weight.ToString
        End If
    End Sub

    Private Sub txtoptTol_Click(sender As Object, e As EventArgs) Handles txtoptTol.Click
        Dim optTol As Double = 0.0000001#
        If TypeBox(sa("Threshold value for optimizer convergence (smaller values are slower, but more accurate):", vbCrLf), optTol, False,, 0#, MaxDouble,,,,,,, optTol.ToString) Then
            txtoptTol.Text = optTol.ToString
        End If
    End Sub

    Private Sub txtmemorySize_Click(sender As Object, e As EventArgs) Handles txtmemorySize.Click
        Dim memorySize As Double = 20.0#
        If TypeBox(sa("Memory size for L-BFGS, specifying the number of past positions and gradients to store for the computation of the next step:", vbCrLf), memorySize, False,, 1.0#, MaxDouble,,,,,,, memorySize.ToString) Then
            txtmemorySize.Text = memorySize.ToString
        End If
    End Sub

    Private Sub txtinitWtsScale_Click(sender As Object, e As EventArgs) Handles txtinitWtsScale.Click
        Dim initWtsScale As Double = 0.0#
        If TypeBox(sa("Sets the initial weights diameter that specifies the range from which values are drawn for the initial weights.{0}These weights are initialized randomly from within this range:", vbCrLf), initWtsScale, False,, 0.0#, MaxDouble,,,,,,, initWtsScale.ToString) Then
            txtinitWtsScale.Text = initWtsScale.ToString
        End If
    End Sub

    Private Sub txtmaxIterations_Click(sender As Object, e As EventArgs) Handles txtmaxIterations.Click
        Dim maxIterations As Integer = 100
        If TypeBox(sa("Sets the maximum number of iterations.{0}After this number of steps, the algorithm stops even if it has not satisfied convergence criteria:", vbCrLf), maxIterations, False,, 1, MaxInteger,,,, maxIterations.ToString) Then
    ...
    Private Sub cbnormalize_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbnormalize.SelectedIndexChanged
        If Not isInitialising Then chknormalize.Checked = True
    End Sub

    Private Sub txtsgdInitTol_TextChanged(sender As Object, e As EventArgs) Handles txtsgdInitTol.TextChanged
        If Not isInitialising Then chksgdInitTol.Checked = True
    End Sub

    Private Sub txtl2Weight_TextChanged(sender As Object, e As EventArgs) Handles txtl2Weight.TextChanged
        If Not isInitialising Then chkl2Weight.Checked = True
    End Sub

    Private Sub txtl1Weight_TextChanged(sender As Object, e As EventArgs) Handles txtl1Weight.TextChanged
        If Not isInitialising Then chkl1Weight.Checked = True
    End Sub

    Private Sub txtoptTol_TextChanged(sender As Object, e As EventArgs) Handles txtoptTol.TextChanged
        If Not isInitialising Then chkoptTol.Checked = True
    End Sub

    Private Sub txtmemorySize_TextChanged(sender As Object, e As EventArgs) Handles txtmemorySize.TextChanged
        If Not isInitialising Then chkmemorySize.Checked = True
    End Sub

    Private Sub txtinitWtsScale_TextChanged(sender As Object, e As EventArgs) Handles txtinitWtsScale.TextChanged
        If Not isInitialising Then If Not isInitialising Then chkinitWtsScale.Checked = True
    End Sub

    Private Sub txtmaxIterations_TextChanged(sender As Object, e As EventArgs) Handles txtmaxIterations.TextChanged
        If Not isInitialising Then chkmaxIterations.Checked = True
    End Sub
End Class