...
#Region "Functions"
    ...
    Private Sub CheckModelExists()
        If File.Exists(doProperFileName(strModelsPath & "EnsembleofDecisionTreesModel.RDS")) Then
            ModelExists = True
...
#End Region
    ...
    Private Sub frmEnsembleofDecisionTrees_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            'initialization
            Call EnsembleofDecisionTrees_Language(Me)
            frmSkin(Me, False)
            ...
            fswModelFileExists.Filter = "EnsembleofDecisionTreesModel.RDS"
                                ...
                                If CheckedColumnsExistInTest Then
                                    ...
                                    For i As Integer = 0 To ColumnNamesFormula.Count - 1
                                        ...
                                        If chkColumnsCombinations.Checked OrElse chkUpToNGramsN.Checked Then
                                            ResultsFileName = sa("EnsembleofDecisionTrees_Results_{0}.csv", (i + 1))
                                            predVarName_ForMultiROC.Add(sa("Tree_PredictionReal{0}", (i + 1)))
                                        Else
                                            ResultsFileName = "EnsembleofDecisionTrees_Results.csv"
                                        End If

                                        Dim SinkFilePathLinux As String = doProperFileNameLinux(strSinkFile)
                                        If RSource({strFunctions & "3.8 Ensemble of Decision Trees.R"}, , {"{reportProgress}", If(chkreportProgress.Checked, txtReportProgress.Text, "rxGetOption('reportProgress')"),
                                                                                                ...
                                                                                                "{6}", "EnsembleofDecisionTreesModel",
