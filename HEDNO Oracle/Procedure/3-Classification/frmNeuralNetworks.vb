...
Public Class frmNeuralNetworks
    Public strLanguage_NeuralNetworks As String()
    Public strLanguage_NeuralNetworks_Tips As String()
...
#Region "Functions"
    Private Sub UpdateCombinations()
        If Not isInitialising Then
            ...
            If clbColumns.CheckedIndices.Count <= 1 Or Not chkColumnsCombinations.Checked Then
                lblCombinationsCount.Text = "1" & strLanguage_NeuralNetworks(19) ' Combination
            Else
                If chkUpToNGramsN.Checked Then
                    ...
                    Next
                    lblCombinationsCount.Text = (CombSum).ToString("n0") & strLanguage_NeuralNetworks(20) ' Combinations
                Else
                    lblCombinationsCount.Text = (NChooseK(clbColumns.CheckedIndices.Count, CInt(txtNGrams.Text))).ToString("n0") & strLanguage_NeuralNetworks(20) ' Combinations
                End If
            End If
        End If
    End Sub
    ...
    Private Sub CheckModelExists()
        If File.Exists(doProperFileName(strModelsPath & "NeuralNetworksModel.RDS")) Then
            ModelExists = True
...
#End Region
    ...
    Private Shadows Sub FormClosing(ByVal sender As Object, ByVal e As ComponentModel.CancelEventArgs) Handles MyBase.Closing
        Try
            If FuncInProgress.Count <> 0 Then
                ...
                If MsgBox(sa(strLanguage_NeuralNetworks(30), vbCrLf, ArrayBox(False, ";", 0, True, FuncInProgress)), MsgBoxStyle.YesNoCancel) <> MsgBoxResult.Yes Then
                    e.Cancel = True
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub frmNeuralNetworks_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            'initialization
            Call NeuralNetworks_Language(Me)
            frmSkin(Me, False)

            cbacceleration.Items.Clear()
            cbacceleration.Items.Add("sse")
            cbacceleration.Items.Add("gpu")
            cbacceleration.SelectedItem = cbacceleration.Items(0)
            ...
            If Not File.Exists(strXDF & "Training_DS.xdf") OrElse Not File.Exists(strXDF & "Test_DS.xdf") Then
                If File.Exists(strXDF & "Classification_DS.xdf") OrElse File.Exists(strXDF & "Clustering_DS.xdf") Then
                    ...
                    If File.Exists(strXDF & "Classification_DS.xdf") Then
                        If File.Exists(strXDF & "Training_DS.xdf") OrElse File.Exists(strXDF & "Test_DS.xdf") Then
                            ...
                            MsgBox(sa(strLanguage_NeuralNetworks(33), vbCrLf, strXDF))
                        Else
                            ...
                            MsgBox(sa(strLanguage_NeuralNetworks(34), vbCrLf, strXDF))
                        End If
                    ...
                    ElseIf File.Exists(strXDF & "Clustering_DS.xdf") Then
                        ...
                        MsgBox(sa(strLanguage_NeuralNetworks(35), vbCrLf, strXDF))
                    End If
                ...
                Else
                    ...
                    MsgBox(sa(strLanguage_NeuralNetworks(36), vbCrLf, strXDF, RemMniHotLetter(frmMain.mniClustering), RemMniHotLetter(frmMain.mniFormTrainAndTestSets), RemMniHotLetter(frmMain.mniClassification)))
            ...
            fswModelFileExists.Filter = "NeuralNetworksModel.RDS"
    ...
    Private Sub btnRunModel_Click(sender As Object, e As EventArgs) Handles btnRunModel.Click
        Try
            If Not isWorking Then
                ...
                (Not chkUseExistingModel.Checked AndAlso Not chkMakePredictions.Checked AndAlso Not chkSavePredictionModel.Checked) Then
                    ...
                    If FuncInProgress.Count = 0 Then
                        ...
                        pbLoading.Visible = True
                        btnRunModel.Text = strLanguage_NeuralNetworks(37) '&Cancel
                        ...
                        FuncInProgress.Add(strLanguage_NeuralNetworks(24)) '"Applying Neural Networks
                        Try
                            ...
                            If RDotNet_Initialization() Then
                                ...
                                If chkacceleration.Checked Then Rdo.Evaluate(sa("acceleration <- '{0}'", cbacceleration.SelectedItem.ToString)) Else Rdo.Evaluate("acceleration <- 'sse'")
                                ...
                                If CheckedColumnsExistInTest Then
                                    ...
                                    For i As Integer = 0 To ColumnNamesFormula.Count - 1
                                        If StopWorking Then
                                            FuncInProgress.Remove(strLanguage_NeuralNetworks(24)) '"Applying Neural Networks
                                            Close()
                                            Exit Sub
                                        End If
                                        WriteToLog({sa("NN_PredictionReal{0}:  {1}",
                                        ...
                                        If chkColumnsCombinations.Checked OrElse chkUpToNGramsN.Checked Then
                                            ResultsFileName = sa("NeuralNetworks_Results_{0}.csv", (i + 1))
                                            predVarName_ForMultiROC.Add(sa("NN_PredictionReal{0}", (i + 1)))
                                        Else
                                            ResultsFileName = "NeuralNetworks_Results.csv"
                                        End If

                                        Dim SinkFilePathLinux As String = doProperFileNameLinux(strSinkFile)
                                        If RSource({strFunctions & "3.9 Neural Networks.R"}, , {"{reportProgress}", If(chkreportProgress.Checked, txtReportProgress.Text, "rxGetOption('reportProgress')"),
                                            ...
                                            If chkStatisticsMode.Checked Then
                                                If chkShowStatistics.Checked Then
                                                    ...
                                                    If Not LabelPredictionExist Then 'If those don't exist, then the Statistics will crash
                                                        ...
                                                        MsgBox(sa(strLanguage_NeuralNetworks(38), vbCrLf, RemCtrHotLetter(chkShowStatistics)))
                                                    Else
                                                        Dim StatisticsResults As GenericVector = Rdo.GetSymbol("StatisticsResults").AsList
                                                    End If

                                                    If chkShowStatistics.Checked Then
                                                        Dim StatisticsForm As New frmStatistics With {.SinkFilePath = SinkFilePathLinux, .ClassificationModelName = "NeuralNetworksModel"}
                                                        StatisticsForm.Show()
                                                    End If
                                                ...
                                                If chkShowROCCurve.Checked Then
                                                    ...
                                                    If Not PredictionRealExist Then 'If this doesn't exist, then the ROC curve will crash
                                                        '         '{1}' was checked, however the Testing Dataset did not have the needed variables.{0}Have you forgotten applying the Predictions from the Machine Learning Algorithm?
                                                        MsgBox(sa(strLanguage_NeuralNetworks(38), vbCrLf, RemCtrHotLetter(chkShowROCCurve)))
                                                    End If
                                                End If
                                            End If
                                            ...
                                            If RDSCreatedOutOfNecessity Then MsgBox(sa(strLanguage_NeuralNetworks(28), vbCrLf, RemCtrHotLetter(chkUseExistingModel)))
                                    ...
                                    If chkColumnsCombinations.Checked AndAlso chkShowROCCurve.Checked Then
                                        ...
                                        If Not RSource({strFunctions & "[Multiple_ROC_Comparisons].R"}, , {"{0}", predVarName,
                                            ...
                                            MsgBox(sa(strLanguage_NeuralNetworks(39), vbCrLf, strXDF & "Test_DS.xdf", predVarName), MsgBoxStyle.Critical)
                                        End If
                                    End If
                                ...
                                Else
                                    ...
                                    MsgBox(sa(strLanguage_NeuralNetworks(40), vbCrLf))
                                End If
                    ...
                    Else
                        ...
                        MsgBox(sa(strLanguage_NeuralNetworks(26), ArrayBox(False, ";", 0, True, FuncInProgress)), MsgBoxStyle.Exclamation) '
                    End If


                Else
                    '         You haven't selected any Dependent Variables, ergo the operation is cancelled
                    MsgBox(sa(strLanguage_NeuralNetworks(41)), MsgBoxStyle.Exclamation)
                End If
            ...
            Else '#If isWorking
                ...
                btnRunModel.Text = strLanguage_NeuralNetworks(42) 'Cancelling...
...
#Region "Misc"
    ...
    Private Sub UpdateSelectAllText()
        If (chkShowDataSummary.Checked OrElse Not chkShowDataSummary.Enabled) AndAlso
           ...
           (chkSavePredictionModel.Checked OrElse Not chkSavePredictionModel.Enabled) Then
            ...
            btnSelectAll.Text = strLanguage_NeuralNetworks(25) 'Unselect &All
        Else
            btnSelectAll.Text = strLanguage_NeuralNetworks(12) 'Select &All
        End If
    End Sub
    Private Sub clbColumns_SelectedIndexChanged(sender As Object, e As EventArgs) Handles clbColumns.SelectedIndexChanged, clbColumns.DoubleClick
        If clbColumns.CheckedIndices.Count = clbColumns.Items.Count Then
            btnSelectAllColumns.Text = strLanguage_NeuralNetworks(43) 'Unselect All &Columns
        Else
            btnSelectAllColumns.Text = strLanguage_NeuralNetworks(29) 'Select &All Columns
        End If
    ...
    Private Sub txtSavePath_Click(sender As Object, e As EventArgs) Handles txtSavePath.Click
        fbdModelPath.Description = strLanguage_NeuralNetworks(44) 'Where would you like your Model to be saved?
...
#End Region
    ...
    Private Sub tmrLoadColumns_Tick(sender As Object, e As EventArgs) Handles tmrLoadColumns.Tick
        Try
            ...
            Catch ex As Exception
                ...
                MsgBox(sa(strLanguage_NeuralNetworks(45), vbCrLf, "TrainingColumnNames", strFunctions & "[ColumnNames_For_Classification].R"))
                Close()
                Exit Sub
            End Try
    ...
    Private Sub txtNGrams_Click(sender As Object, e As EventArgs) Handles txtNGrams.Click
        If clbColumns.CheckedIndices.Count > 0 Then
            ...
            If TypeBox(strLanguage_NeuralNetworks(46), NGram, False,, 1, clbColumns.CheckedIndices.Count,,,,
    ...
    Private Sub txtRoundAt_Click(sender As Object, e As EventArgs) Handles txtRoundAt.Click
        ...
        If TypeBox(strLanguage_NeuralNetworks(17), CurRoundAt, False,, 1, Integer.MaxValue,,,, RoundAt.ToString) Then
            txtRoundAt.Text = CurRoundAt.ToString
        End If
    End Sub
    ...
    Private Sub fswTrainAndTest_Created_Created_Renamed(sender As Object, e As FileSystemEventArgs) Handles fswTrainAndTest.Renamed, fswTrainAndTest.Deleted, fswTrainAndTest.Created
        If e.Name.EndsWith("Training_DS.xdf") OrElse e.Name.EndsWith("Test_DS.xdf") Then
            ...
            Else
                ...
                ShowMTNonInterruptingMsgbox(Me, sa(strLanguage_NeuralNetworks(47), vbCrLf), MsgBoxStyle.Exclamation)
    ...
    Private Sub txtnumLeaves_Click(sender As Object, e As EventArgs) Handles txtnumIterations.Click
        Dim numIterations As Integer = 100
        If TypeBox("The maximum number of leaves (terminal nodes) in any tree:", numIterations, False,, 1, Integer.MaxValue,,,, numIterations.ToString) Then
            txtnumIterations.Text = numIterations.ToString
        End If
    End Sub

    Private Sub txtnTree_Click(sender As Object, e As EventArgs) Handles txtnumHiddenNodes.Click
        Dim numHiddenNodes As Integer = 100
        If TypeBox("Grow this number of trees:", numHiddenNodes, False,, 1, Integer.MaxValue,,,, numHiddenNodes.ToString) Then
            txtnumHiddenNodes.Text = numHiddenNodes.ToString
        End If
    End Sub
    ...
    Private Sub txtnumHiddenNodes_TextChanged(sender As Object, e As EventArgs) Handles txtnumHiddenNodes.TextChanged
        If Not isInitialising Then chknumHiddenNodes.Checked = True
    End Sub

    Private Sub txtnumIterations_TextChanged(sender As Object, e As EventArgs) Handles txtnumIterations.TextChanged
        If Not isInitialising Then chknumIterations.Checked = True
    End Sub

    Private Sub cbacceleration_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbacceleration.SelectedIndexChanged
        If Not isInitialising Then chkacceleration.Checked = True
    End Sub
    ...
    Private Sub txtminiBatchSize_TextChanged(sender As Object, e As EventArgs) Handles txtminiBatchSize.TextChanged
        If Not isInitialising Then chkminiBatchSize.Checked = True
    End Sub
End Class