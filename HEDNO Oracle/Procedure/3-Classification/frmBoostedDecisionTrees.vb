...
Public Class frmBoostedDecisionTrees
    Public strLanguage_BoostedDecisionTrees As String()
    Public strLanguage_BoostedDecisionTrees_Tips As String()
...
#Region "Functions"
    Private Sub UpdateCombinations()
        If Not isInitialising Then
            ...
            If clbColumns.CheckedIndices.Count <= 1 Or Not chkColumnsCombinations.Checked Then
                lblCombinationsCount.Text = "1" & strLanguage_BoostedDecisionTrees(19) ' Combination
            Else
                If chkUpToNGramsN.Checked Then
                    ...
                    Next
                    lblCombinationsCount.Text = (CombSum).ToString("n0") & strLanguage_BoostedDecisionTrees(20) ' Combinations
                Else
                    lblCombinationsCount.Text = (NChooseK(clbColumns.CheckedIndices.Count, CInt(txtNGrams.Text))).ToString("n0") & strLanguage_BoostedDecisionTrees(20) ' Combinations
                End If
            End If
        End If
    End Sub
    ...
    Private Sub CheckModelExists()
        If File.Exists(doProperFileName(strModelsPath & "BoostedDecisionTreesModel.RDS")) Then
            ModelExists = True
...
#End Region
    ...
    Private Shadows Sub FormClosing(ByVal sender As Object, ByVal e As ComponentModel.CancelEventArgs) Handles MyBase.Closing
        Try
            If FuncInProgress.Count <> 0 Then
                ...
                If MsgBox(sa(strLanguage_BoostedDecisionTrees(30), vbCrLf, ArrayBox(False, ";", 0, True, FuncInProgress)), MsgBoxStyle.YesNoCancel) <> MsgBoxResult.Yes Then
                    e.Cancel = True
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub frmBoostedDecisionTrees_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            'initialization
            Call BoostedDecisionTrees_Language(Me)
            frmSkin(Me, False)

            cbunbalancedSets.Items.Clear()
            cbunbalancedSets.Items.Add("False")
            cbunbalancedSets.Items.Add("True")
            cbunbalancedSets.SelectedItem = cbunbalancedSets.Items(1)
            '/initialization
            ...
            If Not File.Exists(strXDF & "Training_DS.xdf") OrElse Not File.Exists(strXDF & "Test_DS.xdf") Then
                If File.Exists(strXDF & "Classification_DS.xdf") OrElse File.Exists(strXDF & "Clustering_DS.xdf") Then
                    ...
                    If File.Exists(strXDF & "Classification_DS.xdf") Then
                        If File.Exists(strXDF & "Training_DS.xdf") OrElse File.Exists(strXDF & "Test_DS.xdf") Then
                            ...
                            MsgBox(sa(strLanguage_BoostedDecisionTrees(33), vbCrLf, strXDF))
                        Else
                            ...
                            MsgBox(sa(strLanguage_BoostedDecisionTrees(34), vbCrLf, strXDF))
                        End If
                    ...
                    ElseIf File.Exists(strXDF & "Clustering_DS.xdf") Then
                        ...
                        MsgBox(sa(strLanguage_BoostedDecisionTrees(35), vbCrLf, strXDF))
                    End If
                ...
                Else
                    ...
                    MsgBox(sa(strLanguage_BoostedDecisionTrees(36), vbCrLf, strXDF, RemMniHotLetter(frmMain.mniClustering), RemMniHotLetter(frmMain.mniFormTrainAndTestSets), RemMniHotLetter(frmMain.mniClassification)))
            ...
            fswModelFileExists.Filter = "BoostedDecisionTreesModel.RDS"
    ...
    Private Sub btnRunModel_Click(sender As Object, e As EventArgs) Handles btnRunModel.Click
        Try
            If Not isWorking Then
                ...
                (Not chkUseExistingModel.Checked AndAlso Not chkMakePredictions.Checked AndAlso Not chkSavePredictionModel.Checked) Then
                    ...
                    If FuncInProgress.Count = 0 Then
                        ...
                        pbLoading.Visible = True
                        btnRunModel.Text = strLanguage_BoostedDecisionTrees(37) '&Cancel
                        ...
                        FuncInProgress.Add(strLanguage_BoostedDecisionTrees(24)) 'Applying Boosted Decision Trees
                        Try
                            Dim TestColumnNames() As String = {}

                            If RDotNet_Initialization() Then
                                If StopWorking Then
                                    FuncInProgress.Remove(strLanguage_BoostedDecisionTrees(24)) 'Applying Boosted Decision Trees
                                    Close()
                                    Exit Sub
                                End If
                                ...
                                If chkunbalancedSets.Checked Then Rdo.Evaluate(sa("unbalancedSets <- {0}", cbunbalancedSets.SelectedItem.ToString)) Else Rdo.Evaluate("unbalancedSets <- FALSE")
                                ...
                                If CheckedColumnsExistInTest Then
                                    ...
                                    For i As Integer = 0 To ColumnNamesFormula.Count - 1
                                        If StopWorking Then
                                            ...
                                            Exit Sub
                                        End If
                                        WriteToLog({sa("BDT_PredictionReal{0}:  {1}",
                                        ...
                                        If chkColumnsCombinations.Checked OrElse chkUpToNGramsN.Checked Then
                                            ResultsFileName = sa("BoostedDecisionTrees_Results_{0}.csv", (i + 1))
                                            predVarName_ForMultiROC.Add(sa("BDT_PredictionReal{0}", (i + 1)))
                                        Else
                                            ResultsFileName = "BoostedDecisionTrees_Results.csv"
                                        End If

                                        Dim SinkFilePathLinux As String = doProperFileNameLinux(strSinkFile)
                                        If RSource({strFunctions & "3.7 Boosted Decision Trees.R"}, , {"{reportProgress}", If(chkreportProgress.Checked, txtReportProgress.Text, "rxGetOption('reportProgress')"),
                                                                                                ...
                                                                                                "{6}", "BoostedDecisionTreesModel",
                                                                                                "{7}", If(chknumTrees.Checked, txtnumTrees.Text, "500"),
                                                                                                "{8}", If(chknumLeaves.Checked, txtnumLeaves.Text, "25"),
                                                                                                "{9}", If(chkgainConfLevel.Checked, txtgainConfLevel.Text, "0")
                                                                                                }, True) Then
                                            ...
                                            If chkStatisticsMode.Checked Then
                                                If chkShowStatistics.Checked Then
                                                    ...
                                                    If Not LabelPredictionExist Then 'If those don't exist, then the Statistics will crash
                                                        ...
                                                        MsgBox(sa(strLanguage_BoostedDecisionTrees(38), vbCrLf, RemCtrHotLetter(chkShowStatistics)))
                                                    Else
                                                        Dim StatisticsResults As GenericVector = Rdo.GetSymbol("StatisticsResults").AsList
                                                    End If

                                                    If chkShowStatistics.Checked Then
                                                        Dim StatisticsForm As New frmStatistics With {.SinkFilePath = SinkFilePathLinux, .ClassificationModelName = "BoostedDecisionTreesModel"}
                                                        StatisticsForm.Show()
                                                    End If
                                                ...
                                                If chkShowROCCurve.Checked Then
                                                    ...
                                                    If Not PredictionRealExist Then 'If this doesn't exist, then the ROC curve will crash
                                                        '         '{1}' was checked, however the Testing Dataset did not have the needed variables.{0}Have you forgotten applying the Predictions from the Machine Learning Algorithm?
                                                        MsgBox(sa(strLanguage_BoostedDecisionTrees(38), vbCrLf, RemCtrHotLetter(chkShowROCCurve)))
                                                    End If
                                                End If
                                            End If
                                            ...
                                            If RDSCreatedOutOfNecessity Then MsgBox(sa(strLanguage_BoostedDecisionTrees(28), vbCrLf, RemCtrHotLetter(chkUseExistingModel)))
                                    ...
                                    If chkColumnsCombinations.Checked AndAlso chkShowROCCurve.Checked Then
                                        ...
                                        If Not RSource({strFunctions & "[Multiple_ROC_Comparisons].R"}, , {"{0}", predVarName,
                                            ...
                                            MsgBox(sa(strLanguage_BoostedDecisionTrees(39), vbCrLf, strXDF & "Test_DS.xdf", predVarName), MsgBoxStyle.Critical)
                                        End If
                                    End If
                                ...
                                Else
                                    ...
                                    MsgBox(sa(strLanguage_BoostedDecisionTrees(40), vbCrLf))
                                End If
                    ...
                    Else
                        ...
                        MsgBox(sa(strLanguage_BoostedDecisionTrees(26), ArrayBox(False, ";", 0, True, FuncInProgress)), MsgBoxStyle.Exclamation) '
                    End If


                Else
                    '         You haven't selected any Dependent Variables, ergo the operation is cancelled
                    MsgBox(sa(strLanguage_BoostedDecisionTrees(41)), MsgBoxStyle.Exclamation)
                End If
            ...
            Else '#If isWorking
                ...
                btnRunModel.Text = strLanguage_BoostedDecisionTrees(42) 'Cancelling...
...
#Region "Misc"
    ...
    Private Sub UpdateSelectAllText()
        If (chkShowDataSummary.Checked OrElse Not chkShowDataSummary.Enabled) AndAlso
           ...
           (chkSavePredictionModel.Checked OrElse Not chkSavePredictionModel.Enabled) Then
            ...
            btnSelectAll.Text = strLanguage_BoostedDecisionTrees(25) 'Unselect &All
        Else
            btnSelectAll.Text = strLanguage_BoostedDecisionTrees(12) 'Select &All
        End If
    End Sub
    Private Sub clbColumns_SelectedIndexChanged(sender As Object, e As EventArgs) Handles clbColumns.SelectedIndexChanged, clbColumns.DoubleClick
        If clbColumns.CheckedIndices.Count = clbColumns.Items.Count Then
            btnSelectAllColumns.Text = strLanguage_BoostedDecisionTrees(43) 'Unselect All &Columns
        Else
            btnSelectAllColumns.Text = strLanguage_BoostedDecisionTrees(29) 'Select &All Columns
        End If
    ...
    Private Sub txtSavePath_Click(sender As Object, e As EventArgs) Handles txtSavePath.Click
        fbdModelPath.Description = strLanguage_BoostedDecisionTrees(44) 'Where would you like your Model to be saved?
...
#End Region
    ...
    Private Sub tmrLoadColumns_Tick(sender As Object, e As EventArgs) Handles tmrLoadColumns.Tick
        Try
            ...
            Catch ex As Exception
                ...
                MsgBox(sa(strLanguage_BoostedDecisionTrees(45), vbCrLf, "TrainingColumnNames", strFunctions & "[ColumnNames_For_Classification].R"))
                Close()
                Exit Sub
            End Try
    ...
    Private Sub txtNGrams_Click(sender As Object, e As EventArgs) Handles txtNGrams.Click
        If clbColumns.CheckedIndices.Count > 0 Then
            ...
            If TypeBox(strLanguage_BoostedDecisionTrees(46), NGram, False,, 1, clbColumns.CheckedIndices.Count,,,,
    ...
    Private Sub txtRoundAt_Click(sender As Object, e As EventArgs) Handles txtRoundAt.Click
        ...
        If TypeBox(strLanguage_BoostedDecisionTrees(17), CurRoundAt, False,, 1, Integer.MaxValue,,,, RoundAt.ToString) Then
            txtRoundAt.Text = CurRoundAt.ToString
        End If
    End Sub
    ...
    Private Sub fswTrainAndTest_Created_Created_Renamed(sender As Object, e As FileSystemEventArgs) Handles fswTrainAndTest.Renamed, fswTrainAndTest.Deleted, fswTrainAndTest.Created
        If e.Name.EndsWith("Training_DS.xdf") OrElse e.Name.EndsWith("Test_DS.xdf") Then
            ...
            Else
                ...
                ShowMTNonInterruptingMsgbox(Me, sa(strLanguage_BoostedDecisionTrees(47), vbCrLf), MsgBoxStyle.Exclamation)
    ...
    Private Sub txtnumLeaves_Click(sender As Object, e As EventArgs) Handles txtnumLeaves.Click
        ...
        If TypeBox(strLanguage_BoostedDecisionTrees(60), numLeaves, False,, 1, Integer.MaxValue,,,, numLeaves.ToString) Then
    ...
    Private Sub txtnTree_Click(sender As Object, e As EventArgs) Handles txtnumTrees.Click
        ...
        If TypeBox(strLanguage_BoostedDecisionTrees(61), nTree, False,, 10, Integer.MaxValue,,,, nTree.ToString) Then
    ...
    Private Sub txtgainConfLevel_Click(sender As Object, e As EventArgs) Handles txtgainConfLevel.Click
        ...
        If TypeBox(strLanguage_BoostedDecisionTrees(62), Complexity, False,, 0D, 1D,,,,,,, Complexity.ToString) Then
    ...
    Private Sub cbunbalancedSets_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbunbalancedSets.SelectedIndexChanged
        If Not isInitialising Then chkunbalancedSets.Checked = True
    End Sub
